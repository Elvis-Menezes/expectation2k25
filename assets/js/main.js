...
class ExpectationsWebsite {
  ...
  createFloatingParticle(container) {
    ...
    particle.style.cssText = `
            ...
            width: ${size}px;
            height: ${size}px;
            background: radial-gradient(circle, #841818, transparent);
            border-radius: 50%;
            pointer-events: none;
            z-index: 1;
  ...
  initCyberCursor() {
    ...
    interactiveElements.forEach((element) => {
      ...
      element.addEventListener("mouseleave", () => {
        ...
        cursor.style.borderColor = "#841818";
      });
    });
  }
  ...
  initTypewriterEffect() {
    ...
    typewriterElements.forEach((element) => {
      ...
      element.textContent = "";
      element.style.borderRight = "2px solid #841818";
  ...
  activateEasterEgg() {
    ...
    const colors = ["#841818", "#ff0080", "#ffff00", "#841818", "#841818"];
  ...
  updateScrollProgress() {
    ...
    if (!progressBar) {
      ...
      progressBar.style.cssText = `
                ...
                width: 0%;
                height: 3px;
                background: linear-gradient(90deg, #841818, #ff0080);
                z-index: 10001;
...
(function () {
  ...
  const hideOverlay = () => {
    if (introOverlay) {
      ...
      try {
        // capture the current frame to set as background before pausing/resetting
        captureVideoFrameAndSetBackground();
        introVideo.pause();
        // keep currentTime where it is (do not reset to 0) to keep the frame available
      } catch (e) {}
  ...
  // Capture current video frame and set it as body background (data URL)
  function captureVideoFrameAndSetBackground() {
    try {
      const w = introVideo.videoWidth || introVideo.clientWidth || window.innerWidth;
      const h = introVideo.videoHeight || introVideo.clientHeight || window.innerHeight;
      if (!w || !h) return;
